...
//-la adaptacion a lo bruto falla probalemente en el uso de los punteros
...
void SetupIMU() {
  ...
  Wire.begin();
  //en este script se usa el fifo, y se usa en continuos mode
  //averirugar de fifo mode
  // para ello ver linea 149 en https://github.com/wollewald/MPU9250_WE/blob/main/src/MPU6500_WE.h
  MPU.autoOffsets();
  MPU.enableGyrDLPF();
...
void ReadAccelerometerAndGyroscope(int* new_accelerometer_samples, int* new_gyroscope_samples) {
  ...
  *new_gyroscope_samples = 0;
  ...
  current_gyroscope_data[0] = gyro.x;
  current_gyroscope_data[1] = gyro.y;
  current_gyroscope_data[2] = gyro.z;
  ...
  *new_gyroscope_samples += 1;
  ...
  current_acceleration_data[0] = acc.x;
  current_acceleration_data[1] = acc.y;
  current_acceleration_data[2] = acc.z;
...
bool IsMoving(int samples_before) {
  ...
  //esta tecnica es muy similar a la que queria utilizar con GVF
...
void UpdateStroke(int new_samples, bool* done_just_triggered) {
  ...
  //Serial.println("UpdateStroke");
  Serial.println(new_samples);
...
void loop() {
  ...
  */
  ...
  //printear(acc);
  Serial.println("-------------");
  Serial.println(accelerometer_samples_read);
  Serial.println(gyroscope_samples_read);
